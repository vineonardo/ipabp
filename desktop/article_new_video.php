<?php
	include 'header.php';
?>

<div class="uk-container uk-text-normal uk-margin-top breadcrumbs">
	<?php include '_breadcrumb.php'; ?>
</div>

<section class="uk-position-relative">
	<div class="uk-container content">
		<div><?php include '_social_share.php'; ?></div>
		<div class="uk-grid-small" uk-grid> 
			 <div class="uk-width-expand uk-position-relative">
			 	<?php include '_social_share_bw.php'; ?>
			 		<div class="section_title">
						<h2>विश्व</h2>
					</div>
					<div class="article_content">
						<h1 class="fz40 martel">राष्ट्रपति ट्रंप ने फिर दोहराई मध्यस्थता की बात, कहा- अगर दोनों देश मानेंगे तो मैं मदद करूंगा</h1>
						<div>

							<p class="live-tv-label fz26">&nbsp;अमेरिकी राष्ट्रपति डॉनल्ड ट्रंप पिछले 3 महीने में 6 बार कश्मीर के मामले में मध्यस्थता की पेशकश कर चुके हैं.</p>
							<p class="live-tv-label fz26">&nbsp;ट्रंप ने कहा कि कश्मीर मामले में वो मदद करना चाहते हैं लेकिन ऐसा वो तभी करेंगे जब भारत इसके लिए तैयार हो.</p>
						</div>
						<div class="article_author fz16 pt-10"><span>By : ABP News</span> <span>|</span> <span>Updated: 24 Oct 2019 6:10PM</span> </div>

						<div class="news_featured">
							<a href="#"><img class="article_feature uk-border-remove" src="images/16-9.jpg" width="100%"></a>
							<div class="news_content"><a href="#"><p class="fz26">अमेरिकी राष्ट्रपति डॉनल्ड ट्रंप पिछले 3 महीने में 6 बार कश्मीर के मामले में मध्यस्थता</p>
							<p class="article_author fz16"><span>Photograph by : Jon Sno | Reuters </span></p>
							</a></div>
						</div>
					</div>

		

        <div class="box-shadow uk-margin-top uk-border-rounded-normal" uk-slideshow="ratio:16:10.5">

		    <ul class="uk-slideshow-items uk-slideshow-ht">
		    	<li>
		    		<div>
			    		<img class="uk-border-rounded-normal uk-border-rounded-remove-bottom uk-position-relative uk-border-bottom" src="images/16-9.jpg">
			    		<div class="vid_thumbnail uk-text-center">
							<img src="images/artboard_op.png">
						</div>
	                	<div class="news_featured box-shadow-remove">
	                			<p class="fz26">अमेरिकी राष्ट्रपति डॉनल्ड ट्रंप पिछले 3 महीने में 6 बार कश्मीर के मामले में मध्यस्थता</p>
								<p class="article_author fz16"><span>Photograph by : Jon Sno | Reuters </span></p>
						</div>
					</div>
		    	</li>
		    	<li>
		    		<img class="uk-border-rounded-normal uk-border-rounded-remove-bottom uk-position-relative uk-border-bottom" src="images/16-9.jpg">
		    		<div class="vid_thumbnail uk-text-center">
							<img src="images/artboard_op.png">
						</div>
                	<div class="news_featured box-shadow-remove">
                			<p class="fz26">अमेरिकी राष्ट्रपति डॉनल्ड ट्रंप पिछले 3 महीने में 6 बार कश्मीर के मामले में मध्यस्थता</p>
							<p class="article_author fz16"><span>Photograph by : Jon Sno | Reuters </span></p>
					</div>
		    	</li>
		    	<li>
		    		<img class="uk-border-rounded-normal uk-border-rounded-remove-bottom uk-position-relative uk-border-bottom" src="images/16-9.jpg">
		    		<div class="vid_thumbnail uk-text-center">
							<img src="images/artboard_op.png">
						</div>
                	<div class="news_featured box-shadow-remove">
                			<p class="fz26">अमेरिकी राष्ट्रपति डॉनल्ड ट्रंप पिछले 3 महीने में 6 बार कश्मीर के मामले में मध्यस्थता</p>
							<p class="article_author fz16"><span>Photograph by : Jon Sno | Reuters </span></p>
					</div>
		    	</li>
		    	<li>
		    		<img class="uk-border-rounded-normal uk-border-rounded-remove-bottom uk-position-relative uk-border-bottom" src="images/16-9.jpg">
		    		<div class="vid_thumbnail uk-text-center">
							<img src="images/artboard_op.png">
						</div>
                	<div class="news_featured box-shadow-remove">
                			<p class="fz26">अमेरिकी राष्ट्रपति डॉनल्ड ट्रंप पिछले 3 महीने में 6 बार कश्मीर के मामले में मध्यस्थता</p>
							<p class="article_author fz16"><span>Photograph by : Jon Sno | Reuters </span></p>
					</div>
		    	</li>
		    	<li>
		    		<img class="uk-border-rounded-normal uk-border-rounded-remove-bottom uk-position-relative uk-border-bottom" src="images/16-9.jpg">
		    		<div class="vid_thumbnail uk-text-center">
							<img src="images/artboard_op.png">
						</div>
                	<div class="news_featured box-shadow-remove">
                			<p class="fz26">अमेरिकी राष्ट्रपति डॉनल्ड ट्रंप पिछले 3 महीने में 6 बार कश्मीर के मामले में मध्यस्थता</p>
							<p class="article_author fz16"><span>Photograph by : Jon Sno | Reuters </span></p>
					</div>
		    	</li>
		    </ul>

		    <div>
	        	<div id="slider_panel" class="uk-position-relative uk-visible-toggle uk-light slider-panel pb-15" tabindex="-1" uk-slider="autoplay: true">
				    <ul class="uk-slider-items uk-grid-small">
				        <li class="uk-width-2-5">
				            <div class="uk-panel">
				                <a href="#"><img class="uk-border-remove uk-border-rounded-normal" uk-slideshow-item="0" src="images/16-9.jpg" alt=""></a>
				                <div class="vid_thumbnail uk-text-center" uk-slideshow-item="0">
									<img src="images/artboard_op.png" width="10%">
								</div>
				            </div>
				        </li>
				        <li class="uk-width-2-5">
				            <div class="uk-panel">
				                  <a href="#"><img class="uk-border-remove uk-border-rounded-normal" uk-slideshow-item="1" src="images/16-9.jpg" alt=""></a>
				                  <div class="vid_thumbnail uk-text-center" uk-slideshow-item="1">
									<img src="images/artboard_op.png" width="10%">
								</div>
				            </div>
				        </li>
				        <li class="uk-width-2-5">
				            <div class="uk-panel">
				                  <a href="#"><img class="uk-border-remove uk-border-rounded-normal" uk-slideshow-item="2" src="images/16-9.jpg" alt=""></a>
				                  <div class="vid_thumbnail uk-text-center" uk-slideshow-item="2">
									<img src="images/artboard_op.png" width="10%">
								</div>
				            </div>
				        </li>
				        <li class="uk-width-2-5">
				            <div class="uk-panel">
				                 <a href="#"><img class="uk-border-remove uk-border-rounded-normal" uk-slideshow-item="3" src="images/16-9.jpg" alt=""></a>
				                 <div class="vid_thumbnail uk-text-center" uk-slideshow-item="3">
									<img src="images/artboard_op.png" width="10%">
								</div>
				            </div>
				        </li>
				        <li class="uk-width-2-5">
				            <div class="uk-panel">
				                <a href="#"><img class="uk-border-remove uk-border-rounded-normal" uk-slideshow-item="4" src="images/16-9.jpg" alt=""></a>
				                <div class="vid_thumbnail uk-text-center" uk-slideshow-item="4">
									<img src="images/artboard_op.png" width="10%">
								</div>
				            </div>
				        </li>
				    </ul>
				   <!--  <a class="uk-position-center-left uk-position-small uk-hidden-hover" href="#" uk-slidenav-previous uk-slider-item="previous"></a>
				    <a class="uk-position-center-right uk-position-small uk-hidden-hover" href="#" uk-slidenav-next uk-slider-item="next"></a> -->
				</div>
        	</div>


		</div>

					<div class="article_data">
						<p class="fz24"><span>न्यूयॉर्क : </span>अमेरिका के राष्ट्रपति डोनल्ड ट्रंप एक बार फिर भारत पाकिस्तान के रिश्ते पर बोले हैं. ट्रंप ने एक बार फिर दोनों देशों के बीच मध्यस्थता की बात दोहराई है. ट्रंप ने कहा है कि भारत और पाकिस्तान की सोच काफी अलग है जिसकी वजह से मुझे चिंता है. लेकिन अगर दोनों मानेंगे तो मैं मदद करूंगा और उन्हें मेरी मदद पसंद भी आएगी. राष्ट्रपति ट्रंप ने कहा, ''कल प्रधानमंत्री इमरान खान से अच्छी मुलाकात हुई, वो लोग बात कर रहे हैं. भारत चाहेगा तो मैं मदद करूंगा. दोनों देशों को मेरी मदद पंद आएगी लेकिन पहले दोनों मानें तो सही. दोनों देशों के विचार अलग हैं, इसे लेकर मैं चिंतित हूं.'' बता दें कि यह छठा मौका है जब राष्ट्रपति ट्रंप ने दोनों देशों के बीच मध्यस्थता की बात कही है.</p>
						
						<p class="fz24">इससे पहले पीएम मोदी ने इस ऐतिहासिक पल में देशवासियों को गवाह बनने की अपील की है. प्रधानमंत्री मोदी ने वैज्ञानिकों को बधाई दी है और देश के लोगों से अनुरोध किया कि सभी देर रात चंद्रयान-2 की लैंडिंग देखें और इस दौरान अपनी तस्वीर क्लिक कर ट्वीट करें. पीएम मोदी का कहना है कि वह उन तस्वीरों को रिट्वीट करेंगे.</p>

						<p class="fz24">बता दें कि कल पाक पीएम इमरान खान से राष्ट्रपति ट्रंप की मुलाकात के दौरान कश्मीर का मुद्दा उठा था लेकिन उन्हें मुहं की खानी पड़ी थी. पाकिस्तान के प्रधानमंत्री को डोनाल्ड ट्रंप ने झटका देते हुए साफ किया कि वो तैयार तो हैं लेकिन भारत की सहमति भी जरूरी है. जाहिर है कि भारत, कश्मीर को द्विपक्षीय मुद्दा मानता है और ये साफ है कि भारत इसमें किसी तीसरे की भूमिका नहीं मानता है. ट्रंप ने कहा कि अकेले इमरान खान के तैयार होने से कुछ नहीं होता, दूसरे पक्ष का तैयार होना भी जरूरी है. इसके साथ ही उन्होंने कहा कि उम्मीद है कि भारत-पाक इसे मिलकर सुलझाएंगे.</p>
						<p class="fz24"><span>इमरान के सामने ट्रंप ने किया हाउडी मोदी का जिक्र</span></p>
						<p class="fz24">इससे पहले पीएम मोदी ने इस ऐतिहासिक पल में देशवासियों को गवाह बनने की अपील की है. प्रधानमंत्री मोदी ने वैज्ञानिकों को बधाई दी है और देश के लोगों से अनुरोध किया कि सभी देर रात चंद्रयान-2 की लैंडिंग देखें और इस दौरान अपनी तस्वीर क्लिक कर ट्वीट करें. पीएम मोदी का कहना है कि वह उन तस्वीरों को रिट्वीट करेंगे.</p>

						<p class="fz24"><span>इमरान के सामने ट्रंप ने किया हाउडी मोदी का जिक्र</span></p>
						<p class="fz24">बता दें कि कल पाक पीएम इमरान खान से राष्ट्रपति ट्रंप की मुलाकात के दौरान कश्मीर का मुद्दा उठा था लेकिन उन्हें मुहं की खानी पड़ी थी. पाकिस्तान के प्रधानमंत्री को डोनाल्ड ट्रंप ने झटका देते हुए साफ किया कि वो तैयार तो हैं लेकिन भारत की सहमति भी जरूरी है. जाहिर है कि भारत, कश्मीर को द्विपक्षीय मुद्दा मानता है और ये साफ है कि भारत इसमें किसी तीसरे की भूमिका नहीं मानता है. ट्रंप ने कहा कि अकेले इमरान खान के तैयार होने से कुछ नहीं होता, दूसरे पक्ष का तैयार होना भी जरूरी है. इसके साथ ही उन्होंने कहा कि उम्मीद है कि भारत-पाक इसे मिलकर सुलझाएंगे.</p>
						<p class="fz24">इससे पहले पीएम मोदी ने इस ऐतिहासिक पल में देशवासियों को गवाह बनने की अपील की है. प्रधानमंत्री मोदी ने वैज्ञानिकों को बधाई दी है और देश के लोगों से अनुरोध किया कि सभी देर रात चंद्रयान-2 की लैंडिंग देखें और इस दौरान अपनी तस्वीर क्लिक कर ट्वीट करें. पीएम मोदी का कहना है कि वह उन तस्वीरों को रिट्वीट करेंगे.</p>

						
					</div>

					<div class="article_block_quote fz32 uk-text-bold"><span class="abp_icon_tr abp_quotes_left"></span>&nbsp;&nbsp; मैं वहां बैठा था. मैंने भारत और प्रधानमंत्री की तरफ से बहुत की आक्रामक बयान सुना. मैं ये कहूंगा कि वहां 59000 लोगों का अच्छा समर्थन मिला.’’ इमरान खान ने पत्रकारों से कहा कि प्रधानमंत्री मोदी और इमरान खान दोनों से उनके अच्छे संबंध हैं.<span class="abp_icon_tr abp_quotes_right"></span>
					<div><em> - रवि प्रसाद</em></div></div>


					<div class="article_featured2">
						<div class="article_featured1_question fz26 uk-position-relative">
							<span class="abp_icon_tr abp_article_arrow_left abp_article_arrow"></span>मनोज तिवारी ने कहा है कि अपनी गलतियों को छिपाने के लिए केजरीवाल पड़ोसी राज्यों को जिम्मेदार ठहरा रहे हैं. ?
						</div>
						<div class="news_featured article_padding">
							<div><p class="fz26"><span class="abp_icon_tr abp_article_arrow abp_article_arrow_right"></span>बता दें कि कल पाक पीएम इमरान खान से राष्ट्रपति ट्रंप की मुलाकात के दौरान कश्मीर का मुद्दा उठा था लेकिन उन्हें मुहं की खानी पड़ी थी. पाकिस्तान के प्रधानमंत्री को डोनाल्ड ट्रंप ने झटका देते हुए साफ किया कि वो तैयार तो हैं लेकिन भारत की सहमति भी जरूरी है. जाहिर है कि भारत, कश्मीर को द्विपक्षीय मुद्दा मानता है और ये साफ है कि भारत इसमें किसी तीसरे की भूमिका नहीं मानता है. ट्रंप ने कहा कि अकेले इमरान खान के तैयार होने से कुछ नहीं होता, दूसरे पक्ष का तैयार होना भी जरूरी है. इसके साथ ही उन्होंने कहा कि उम्मीद है कि भारत-पाक इसे मिलकर सुलझाएंगे.</p></div>
						</div>
					</div>


					<div class="article_featured2">
						<div class="news_featured article_padding">
							<div>
								<p class="fz32 uk-text-bold uk-remove-padding" style="margin-bottom: 15px;border-bottom:2px solid #EC2346;"><span class="red-label">1. </span>मैं वहां बैठा था. मैंने भारत और प्रधानमंत्री की तरफ से बहुत की आक्रामक बयान सुना. मैं ये कहूंगा कि वहां</p>
								<p class="fz26">बता दें कि कल पाक पीएम इमरान खान से राष्ट्रपति ट्रंप की मुलाकात के दौरान कश्मीर का मुद्दा उठा था लेकिन उन्हें मुहं की खानी पड़ी थी. पाकिस्तान के प्रधानमंत्री को डोनाल्ड ट्रंप ने झटका देते हुए साफ किया कि वो तैयार तो हैं लेकिन भारत की सहमति भी जरूरी है. जाहिर है कि भारत, कश्मीर को द्विपक्षीय मुद्दा मानता है और ये साफ है कि भारत इसमें किसी तीसरे की भूमिका नहीं मानता है. ट्रंप ने कहा कि अकेले इमरान खान के तैयार होने से कुछ नहीं होता, दूसरे पक्ष का तैयार होना भी जरूरी है. इसके साथ ही उन्होंने कहा कि उम्मीद है कि भारत-पाक इसे मिलकर सुलझाएंगे.</p>
						</div>
						</div>
					</div>

					<div class="article_featured2">
						<div class="news_featured">
							<div>
								<p class="fz32 uk-text-bold uk-remove-padding"><span class="red-label">2. </span>मैं वहां बैठा था. मैंने भारत और प्रधानमंत्री की तरफ से बहुत की आक्रामक बयान सुना. मैं ये कहूंगा कि वहां</p>
							</div>
							<a href="#"><img class="uk-border-rounded-remove-top" src="images/16-9.jpg" width="100%"></a>
							<div>
								<p class="fz26">बता दें कि कल पाक पीएम इमरान खान से राष्ट्रपति ट्रंप की मुलाकात के दौरान कश्मीर का मुद्दा उठा था लेकिन उन्हें मुहं की खानी पड़ी थी.</p>
							</div>
						</div>
					</div>

					<div class="article_featured2">
						<div class="news_featured article_padding">
							<div>
								<p class="fz32 uk-border-bottom uk-text-bold uk-remove-padding"><span class="red-label">POLL : </span>मैं वहां बैठा था. मैंने भारत और प्रधानमंत्री की तरफ से बहुत की आक्रामक बयान सुना. मैं ये कहूंगा वहां</p>
							</div>

							<div class="form_section">
									<form>
										<div class="form_radioBtn">
										<div class="form_radioInput"><label><input type="radio" name="article_rad" /> Business</label></div>
										<div class="form_radioInput"><label><input type="radio" name="article_rad" /> Lifestyle</label></div>
										<div class="form_radioInput"><label><input type="radio" name="article_rad" /> Sports</label></div>
										<div class="form_radioInput"><label><input type="radio" name="article_rad" /> Technology</label></div>
										</div>
										<div class="num_votes"><span>2000 Votes</span> <span>|</span> <span>Updated: 24 Oct 2019 6:06PM</span> </div>
										<div class="form_submit pt-10"><input type="submit" name="Submit" />
										</div>
									</form>
							</div>
							
						</div>
					</div>


					<div class="article_featured2">
						<div class="news_featured article_padding">
							<div>
								<p  class="fz32 uk-text-bold uk-border-bottom"><span class="red-label">QUIZ : </span>मैं वहां बैठा था. मैंने भारत और प्रधानमंत्री की तरफ से बहुत की आक्रामक बयान सुना. मैं ये कहूंगा वहां</p>
							</div>

							<div class="form_section">
									<form>
										<div class="form_radioBtn">
										<div class="form_radioInput">
											<textarea style="width:100%;border: none;outline: none;" rows="8" placeholder="Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod dolor ipsum dolor sit amet,ipsum dolor sit sit amet"></textarea>
										</div>
										</div>
										<div class="num_votes"><span>745 Anwers</span> <span>|</span> <span>Updated: 24 Oct 2019 6:06PM</span> </div>
										<div class="form_submit pt-10"><input type="submit" name="Submit" />
										</div>
									</form>
							</div>
							
						</div>
					</div>




					
					

			 </div>
			 <div class="uk-sidebar">
			 	<?php include '_adBanner_square.php'; ?>

				<div class="section_title uk-margin-top">
					<h2>रिलेटेड स्टोरीज</h2>
				</div>

							<div class="news_featured">
								<a href="#" title="पहलवान बबीता फोगाट का सब इंस्पेक्टर पद से दिया इस्तीफा मंजूर, चुनाव लड़ने की अटकलें">
									<img src="images/16-9.jpg" width="100%" />
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>

							<div class="other_news">
								<div class="uk-grid-small" uk-grid>
									<div class="uk-width-2-3">
										<a href="#"><p class="fz18">पहलवान बबीता फोगाट का सब इंस्पेक्टर पद से दिया इस्तीफा मंजूर</p></a>
									</div>
									<div class="uk-width-1-3 uk-position-relative uk-padding-remove-left">
										<div class="play_btn_div" style="position: absolute;right: 8px;top:8px;">
											<img class="play_btn_div_small" src="images/artboard.png">
										</div>
										<a href="#"><img src="images/1-1.jpg" width="100%"></a>
									</div>
								</div>
							</div>

							<div class="other_news">
								<div class="uk-grid-small" uk-grid>
									<div class="uk-width-2-3">
										<a href="#"><p class="fz18">पहलवान बबीता फोगाट का सब इंस्पेक्टर पद से दिया इस्तीफा मंजूर</p></a>
									</div>
									<div class="uk-width-1-3 uk-position-relative uk-padding-remove-left">
										<div class="play_btn_div" style="position: absolute;right: 8px;top:8px;">
											<img class="play_btn_div_small" src="images/artboard.png">
										</div>
										<a href="#"><img src="images/1-1.jpg" width="100%"></a>
									</div>
								</div>
							</div>
							<div class="other_news">
								<div class="uk-grid-small" uk-grid>
									<div class="uk-width-2-3">
										<a href="#"><p class="fz18">पहलवान बबीता फोगाट का सब इंस्पेक्टर पद से दिया इस्तीफा मंजूर</p></a>
									</div>
									<div class="uk-width-1-3 uk-position-relative uk-padding-remove-left">
										<div class="play_btn_div" style="position: absolute;right: 8px;top:8px;">
											<img class="play_btn_div_small" src="images/artboard.png">
										</div>
										<a href="#"><img src="images/1-1.jpg" width="100%"></a>
									</div>
								</div>
							</div>
							<div class="other_news">
								<div class="uk-grid-small" uk-grid>
									<div class="uk-width-2-3">
										<a href="#"><p class="fz18">पहलवान बबीता फोगाट का सब इंस्पेक्टर पद से दिया इस्तीफा मंजूर</p></a>
									</div>
									<div class="uk-width-1-3 uk-position-relative uk-padding-remove-left">
										<div class="play_btn_div" style="position: absolute;right: 8px;top:8px;">
											<img class="play_btn_div_small" src="images/artboard.png">
										</div>
										<a href="#"><img src="images/1-1.jpg" width="100%"></a>
									</div>
								</div>
							</div>

			 	<div class="uk-margin-top">
			 	<?php include '_adBanner_square.php'; ?>
				</div>

				<div class="section_title uk-margin-top">
					<h2>ताज़ा वीडियो</h2>
				</div>
				<div class="fresh_videos">
							<div class="news_featured">
								<a href="#" title="बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…">
									<img src="images/16-9.jpg" width="100%" />
									<div class="play_btn_div">
										<img src="images/artboard_op.png">
									</div>
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>
						<div class="news_featured">
								<a href="#" title="बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…">
									<img src="images/16-9.jpg" width="100%" />
									<div class="play_btn_div">
										<img src="images/artboard_op.png">
									</div>
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>
							<div class="news_featured">
								<a href="#" title="बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…">
									<img src="images/16-9.jpg" width="100%" />
									<div class="play_btn_div">
										<img src="images/artboard_op.png">
									</div>
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>
							<div class="news_featured">
								<a href="#" title="बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…">
									<img src="images/16-9.jpg" width="100%" />
									<div class="play_btn_div">
										<img src="images/artboard_op.png">
									</div>
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>
							<div class="news_featured">
								<a href="#" title="बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…">
									<img src="images/16-9.jpg" width="100%" />
									<div class="play_btn_div">
										<img src="images/artboard_op.png">
									</div>
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>
							<div class="uk-margin-top">
							 	<?php include '_adBanner_square.php'; ?>
								</div>
				</div>

							<div class="section_title uk-margin-top">
								<h2>ताज़ा वीडियो</h2>
							</div>


			<div class="fresh_videos">
							<div class="news_featured">
								<a href="#" title="बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…">
									<img src="images/16-9.jpg" width="100%" />
									<div class="play_btn_div">
										<img src="images/artboard_op.png">
									</div>
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>
						<div class="news_featured">
								<a href="#" title="बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…">
									<img src="images/16-9.jpg" width="100%" />
									<div class="play_btn_div">
										<img src="images/artboard_op.png">
									</div>
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>
							<div class="news_featured">
								<a href="#" title="बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…">
									<img src="images/16-9.jpg" width="100%" />
									<div class="play_btn_div">
										<img src="images/artboard_op.png">
									</div>
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>
							<div class="news_featured">
								<a href="#" title="बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…">
									<img src="images/16-9.jpg" width="100%" />
									<div class="play_btn_div">
										<img src="images/artboard_op.png">
									</div>
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>
							<div class="news_featured">
								<a href="#" title="बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…">
									<img src="images/16-9.jpg" width="100%" />
									<div class="play_btn_div">
										<img src="images/artboard_op.png">
									</div>
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>
							<div class="uk-margin-top">
							 	<?php include '_adBanner_square.php'; ?>
								</div>
				</div>

			
			


				<div class="section_title uk-margin-top">
					<h2>रिलेटेड स्टोरीज</h2>
				</div>
							<div class="news_featured">
								<a href="#" title="पहलवान बबीता फोगाट का सब इंस्पेक्टर पद से दिया इस्तीफा मंजूर, चुनाव लड़ने की अटकलें">
									<img src="images/16-9.jpg" width="100%" />
									<div class="play_btn_div">
										<img src="images/artboard_op.png">
									</div>
									<div class="news_content">
										<p class="fz18 p-10">बेहद खूबसूरत साड़ी में गणपति दर्शन को पहुंची दीपिका पादुकोण…</p>
									</div>
								</a>
							</div>

							<div class="other_news">
								<div class="uk-grid-small" uk-grid>
									<div class="uk-width-2-3">
										<a href="#"><p class="fz18">पहलवान बबीता फोगाट का सब इंस्पेक्टर पद से दिया इस्तीफा मंजूर</p></a>
									</div>
									<div class="uk-width-1-3 uk-position-relative uk-padding-remove-left">
										<div class="play_btn_div" style="position: absolute;right: 8px;top:8px;">
											<img class="play_btn_div_small" src="images/artboard.png">
										</div>
										<a href="#"><img src="images/1-1.jpg" width="100%"></a>
									</div>
								</div>
							</div>

							<div class="other_news">
								<div class="uk-grid-small" uk-grid>
									<div class="uk-width-2-3">
										<a href="#"><p class="fz18">पहलवान बबीता फोगाट का सब इंस्पेक्टर पद से दिया इस्तीफा मंजूर</p></a>
									</div>
									<div class="uk-width-1-3 uk-position-relative uk-padding-remove-left">
										<div class="play_btn_div" style="position: absolute;right: 8px;top:8px;">
											<img class="play_btn_div_small" src="images/artboard.png">
										</div>
										<a href="#"><img src="images/1-1.jpg" width="100%"></a>
									</div>
								</div>
							</div>

							<div class="other_news">
								<div class="uk-grid-small" uk-grid>
									<div class="uk-width-2-3">
										<a href="#"><p class="fz18">पहलवान बबीता फोगाट का सब इंस्पेक्टर पद से दिया इस्तीफा मंजूर</p></a>
									</div>
									<div class="uk-width-1-3 uk-position-relative uk-padding-remove-left">
										<div class="play_btn_div" style="position: absolute;right: 8px;top:8px;">
											<img class="play_btn_div_small" src="images/artboard.png">
										</div>
										<a href="#"><img src="images/1-1.jpg" width="100%"></a>
									</div>
								</div>
							</div>

							<div class="other_news">
								<div class="uk-grid-small" uk-grid>
									<div class="uk-width-2-3">
										<a href="#"><p class="fz18">पहलवान बबीता फोगाट का सब इंस्पेक्टर पद से दिया इस्तीफा मंजूर</p></a>
									</div>
									<div class="uk-width-1-3 uk-position-relative uk-padding-remove-left">
										<div class="play_btn_div" style="position: absolute;right: 8px;top:8px;">
											<img class="play_btn_div_small" src="images/artboard.png">
										</div>
										<a href="#"><img src="images/1-1.jpg" width="100%"></a>
									</div>
								</div>
							</div>
							<div class="uk-margin-top">
						 		<?php include '_adBanner_square.php'; ?>
							</div>
				
			 </div>
		</div>
	</div>
</section>

<section>
	<div class="uk-container">
		<div uk-grid> 
			 <div class="uk-width-expand">
			 	<?php include '_tag.php'; ?>
				<?php include '_comments.php'; ?>
			 </div>
			 <div class="uk-sidebar">
			 </div>
			</div></div></section>

<?php
	include 'footer.php';
?>