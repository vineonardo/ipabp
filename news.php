<!DOCTYPE html>
<html>
<head>
    <title>ABP News</title>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <link rel="stylesheet" href="css/custom.min.css">
    <link rel="icon" type="image/x-icon" href="https://static.abplive.com/frontend/abplive/images/favicon.ico">
    <link rel="shortcut icon" type="image/x-icon" href="https://static.abplive.com/frontend/abplive/images/favicon.ico">
    <link rel="stylesheet" href="css/override.css" />
    <!-- post development, override.css's content should be placed in Head tag directly with minification -->
</head>
<body>
<div id="wrapper">
    <?php include '_header.php'; ?>
    <div id="content">
        
        <div class="uk-padding-small uk-background-muted uk-padding-remove-vertical"><br/>
            <div class="uk-card uk-card-default box-shadow">
                <div class="uk-grid uk-grid-small uk-flex uk-flex-middle uk-padding-smaller">
                    <div>
                        <div class="uk-border-circle uk-overflow-hidden">
                            <img data-src="https://randomuser.me/api/portraits/women/13.jpg" alt="Profile" width="80" height="80" uk-img />    
                        </div>
                    </div>
                    <div class="uk-flex uk-flex-column uk-flex-around">
                        <h4 class="uk-text-normal uk-margin-small">Good Morning Anna</h4>
                        <h5 class="uk-text-small uk-margin-remove">You've 15 new stories</h5>
                        <a href="#" class="uk-margin-small red-label uk-text-small">Customise my Newsfeed</a>
                    </div>
                </div>
            </div>
        </div>
        

        <div class="section uk-padding-small uk-background-muted small-header">

            <div class="uk-flex section-header uk-flex-between">
                <h3 class="section-heading">आपके लिये</h3>
            </div>

            <div class="for-you-featured">
                <div class="uk-position-relative" uk-slider>
                    <ul class="uk-slider-items uk-grid uk-grid-small">
                        <li class="uk-width-4-5">
                            <div class="uk-card box-shadow uk-border-rounded-medium uk-overflow-hidden">
                                <a href="#" title="BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित">
                                <img class="uk-border-bottom" data-src="https://img.etimg.com/thumb/width-640,height-360,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="400" height="300" uk-img/>
                                <h3><span class="red-label uk-text-bold">Politics:</span> BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित</h3></a>
                            </div>
                        </li>
                        <li class="uk-width-4-5">
                            <div class="uk-card box-shadow uk-border-rounded-medium uk-overflow-hidden">
                                <a href="#" title="BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित">
                                <img class="uk-border-bottom" data-src="https://img.etimg.com/thumb/width-640,height-360,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="400" height="300" uk-img/>
                                <h3><span class="red-label uk-text-bold">Politics:</span>BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित</h3></a>
                            </div>
                        </li>
                        <li class="uk-width-4-5">
                            <div class="uk-card box-shadow uk-border-rounded-medium uk-overflow-hidden">
                                <a href="#" title="BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित">
                                <img class="uk-border-bottom" data-src="https://img.etimg.com/thumb/width-640,height-360,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="400" height="300" uk-img/>
                                <h3><span class="red-label uk-text-bold">Politics:</span>BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित</h3></a>
                            </div>
                        </li>
                       
                    </ul>
                </div>
            </div><!-- breaking-featured -->

            <div class="list-wrap opinion-list">
                <ul class="list-side-thumb">
                    <li class="uk-card uk-card-default box-shadow">
                        <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी..." class="uk-padding-remove">
                            <div class="uk-grid uk-grid-smaller uk-grid-match">
                                <div class="uk-width-3-5">
                                    <div class="uk-padding-smaller uk-flex uk-flex-column uk-flex-between">
                                        <div><span class="quote-icon red-label" uk-icon="icon:quote-right;ratio:1.1"></span><span> <span class="red-label uk-text-bold">Opinion: </span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी...</span></div>
                                        <div></div>
                                        <div class="author uk-flex uk-flex-middle">
                                            <div class="avatar uk-border-circle uk-overflow-hidden">
                                                <img src="//placehold.it/50x50" alt="Author" width="24" />
                                            </div>
                                            <span class="uk-text-small">विनय लाल / प्रोफेसर, UCLA</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="uk-width-2-5">
                                    <img class="box-shadow uk-border-rounded-medium uk-overflow-hidden" data-src="https://img.etimg.com/thumb/width-150,height-150,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="150" height="150" uk-img/>
                                </div>
                            </div>
                        </a>
                    </li>
                </ul>
            </div><!-- list-wrap -->
        </div><!-- section -->


        <div class="section uk-padding-small">
            <div class="list-wrap videos-list uk-margin-remove-top">
                <ul class="list-side-thumb">
                    <li class="uk-margin-remove-top">
                        <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी...">
                            <div class="uk-grid uk-grid-small">
                                <div class="uk-width-3-5"><span class="red-label uk-text-bold">Opinion-</span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी...
                                <div class="watch-btn">
                                    <span class="icon" uk-icon="icon:triangle-right;ratio:1.5"></span>
                                    <span>Watch</span>
                                </div>
                                </div>
                                <div class="uk-width-2-5">
                                    <img class="box-shadow uk-border-rounded-medium uk-overflow-hidden" data-src="https://img.etimg.com/thumb/width-150,height-120,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="150" height="120" uk-img/>
                                </div>
                            </div>
                        </a>
                    </li>
                    <li class="uk-margin-remove-bottom">
                        <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी...">
                            <div class="uk-grid uk-grid-small">
                                <div class="uk-width-3-5"><span class="red-label uk-text-bold">Opinion-</span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी...
                                <div class="watch-btn">
                                    <span class="icon" uk-icon="icon:triangle-right;ratio:1.5"></span>
                                    <span>Watch</span>
                                </div>
                                </div>
                                <div class="uk-width-2-5">
                                    <img class="box-shadow uk-border-rounded-medium uk-overflow-hidden" data-src="https://img.etimg.com/thumb/width-150,height-120,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="150" height="120" uk-img/>
                                </div>
                            </div>
                        </a>
                    </li>
                </ul>
            </div><!-- list-wrap -->
        </div><!-- section -->

        <?php include '_ad300x250.php'; ?>

        <div class="section uk-padding-small">
                <div class="uk-panel uk-border-rounded-medium uk-overflow-hidden box-shadow simple-title">
                    <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत के सारे कोडवर्ड्स बदल दिये" class="uk-display-block">
                        <div class="uk-position-relative">
                            <img class="uk-border-bottom" data-src="https://img.etimg.com/thumb/width-640,height-360,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="400" height="300" uk-img/>
                            <span class="play-icon uk-position-center" uk-icon="icon:triangle-right;ratio:2.0"></span>
                        </div>
                    <h3><span class="red-label uk-text-bold">Sports - </span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत...</h3></a>
                </div>

                <div class="list-wrap videos-list">
                    <ul class="list-side-thumb">
                        <li>
                            <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी...">
                                <div class="uk-grid uk-grid-small">
                                    <div class="uk-width-3-5"><span class="red-label uk-text-bold">Sports - </span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी...
                                    <div class="watch-btn photos">
                                        <span class="icon" uk-icon="icon:image;ratio:0.9"></span>
                                        <span class="uk-dark">07</span> <span>Photos</span>
                                    </div>
                                    </div>
                                    <div class="uk-width-2-5">
                                        <img class="box-shadow uk-border-rounded-medium uk-overflow-hidden" data-src="https://img.etimg.com/thumb/width-150,height-120,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="150" height="120" uk-img/>
                                    </div>
                                </div>
                            </a>
                        </li>
                        <li>
                            <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी...">
                                <div class="uk-grid uk-grid-small">
                                    <div class="uk-width-3-5"><span class="red-label uk-text-bold">Sports - </span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी...</div>
                                    <div class="uk-width-2-5">
                                        <img class="box-shadow uk-border-rounded-medium uk-overflow-hidden" data-src="https://img.etimg.com/thumb/width-150,height-120,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="150" height="120" uk-img/>
                                    </div>
                                </div>
                            </a>
                        </li>
                        <li>
                            <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी...">
                                <div class="uk-grid uk-grid-small">
                                    <div class="uk-width-3-5"><span class="red-label uk-text-bold">Sports - </span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी...
                                    <div class="watch-btn">
                                        <span class="icon" uk-icon="icon:triangle-right;ratio:1.5"></span>
                                        <span>Watch</span>
                                    </div>
                                    </div>
                                    <div class="uk-width-2-5">
                                        <img class="box-shadow uk-border-rounded-medium uk-overflow-hidden" data-src="https://img.etimg.com/thumb/width-150,height-120,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="150" height="120" uk-img/>
                                    </div>
                                </div>
                            </a>
                        </li>
                    </ul>
                </div><!-- list-wrap -->

                <hr class="uk-hr">

                <div class="uk-panel uk-border-rounded-medium uk-overflow-hidden box-shadow simple-title">
                    <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत के सारे कोडवर्ड्स बदल दिये" class="uk-display-block">
                        <div class="uk-position-relative">
                            <img class="uk-border-bottom" data-src="https://img.etimg.com/thumb/width-640,height-360,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="400" height="300" uk-img/>
                            <span class="play-icon uk-position-center" uk-icon="icon:triangle-right;ratio:2.0"></span>
                        </div>
                    <h3><span class="red-label uk-text-bold">Sports - </span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत...</h3></a>
                </div>

                <div class="uk-margin latest-videos small-header">
                <div class="uk-position-relative" uk-slider>
                    <div class="uk-flex section-header uk-flex-between">
                        <h3 class="section-heading">Latest Videos</h3>    
                        <a href="#" class="view-all"><span>View All</span><span class="icon"></span></a>
                    </div>
                    <ul class="uk-slider-items uk-grid uk-grid-small simple-title">
                        <li class="uk-width-4-5">
                            <div class="uk-card box-shadow uk-border-rounded-medium uk-overflow-hidden">
                                <a href="#" title="BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित">
                                <div class="uk-position-relative">
                                    <img class="uk-border-bottom" data-src="https://img.etimg.com/thumb/width-640,height-360,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="400" height="300" uk-img/>
                                    <div class="play-icon uk-position-center" uk-icon="icon:triangle-right;ratio:1.5"></div>
                                </div>
                                <h3>BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित</h3></a>
                            </div>
                        </li>
                        <li class="uk-width-4-5">
                            <div class="uk-card box-shadow uk-border-rounded-medium uk-overflow-hidden">
                                <a href="#" title="BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित">
                                <div class="uk-position-relative">
                                    <img class="uk-border-bottom" data-src="https://img.etimg.com/thumb/width-640,height-360,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="400" height="300" uk-img/>
                                    <div class="play-icon uk-position-center" uk-icon="icon:triangle-right;ratio:1.5"></div>
                                </div>
                                <h3>BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित</h3></a>
                            </div>
                        </li>
                        <li class="uk-width-4-5">
                            <div class="uk-card box-shadow uk-border-rounded-medium uk-overflow-hidden">
                                <a href="#" title="BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित">
                                <div class="uk-position-relative">
                                    <img class="uk-border-bottom" data-src="https://img.etimg.com/thumb/width-640,height-360,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="400" height="300" uk-img/>
                                    <div class="play-icon uk-position-center" uk-icon="icon:triangle-right;ratio:1.5"></div>
                                </div>
                                <h3>BJP संसदीय बोर्ड की बैठक में उठा प्रदूषण का मुद्दा, आज लोकसभा में भी होगी चर्चा, राज्यसभा 2 बजे तक स्थगित</h3></a>
                            </div>
                        </li>
                    </ul>
                </div>
                </div><!-- latest-videos -->
                    

                <div class="uk-panel uk-border-rounded-medium uk-overflow-hidden box-shadow simple-title">
                    <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत के सारे कोडवर्ड्स बदल दिये" class="uk-display-block">
                        <div class="uk-position-relative">
                            <img class="uk-border-bottom" data-src="https://img.etimg.com/thumb/width-640,height-360,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="400" height="300" uk-img/>
                            <span class="play-icon uk-position-center" uk-icon="icon:triangle-right;ratio:2.0"></span>
                        </div>
                    <h3><span class="red-label uk-text-bold">Sports - </span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत...</h3></a>
                </div>

            
                <div class="uk-flex uk-flex-center uk-flex-middle uk-margin">
                    <img src="//placehold.it/300x100&text=300x100 Ad" alt="Ad" />
                </div>

                <div class="uk-card uk-card-default uk-padding-small simple-title">
                        <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत के सारे कोडवर्ड्स बदल दिये" class="uk-display-block">
                            <div class="uk-grid uk-grid-smaller uk-grid-match">
                                <div class="uk-width-2-3">
                                    <div class="uk-border-rounded uk-overflow-hidden">
                                    <img data-src="https://img.etimg.com/thumb/width-400,height-300,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="300" height="250" uk-img/></div>
                                </div>
                                <div class="uk-width-1-3">
                                    <div class="uk-flex uk-flex-column uk-flex-between">
                                        <div class="uk-border-rounded uk-overflow-hidden"><img data-src="https://img.etimg.com/thumb/width-200,height-150,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="200" height="150" uk-img/></div>

                                        <div class="uk-border-rounded uk-overflow-hidden"><img data-src="https://img.etimg.com/thumb/width-200,height-150,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="200" height="150" uk-img/></div>
                                    </div>
                                </div>
                            </div>
                        <h3><span class="red-label uk-text-bold">Photos - </span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत...</h3></a>
                    </div>

                <hr class="uk-hr">

                <div class="uk-panel uk-border-rounded-medium uk-overflow-hidden box-shadow simple-title">
                    <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत के सारे कोडवर्ड्स बदल दिये" class="uk-display-block">
                        <div class="uk-position-relative">
                            <img class="uk-border-bottom" data-src="https://img.etimg.com/thumb/width-640,height-360,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="400" height="300" uk-img/>
                            <span class="play-icon uk-position-center" uk-icon="icon:triangle-right;ratio:2.0"></span>
                        </div>
                    <h3><span class="red-label uk-text-bold">Sports - </span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत...</h3></a>
                </div>

                <hr class="uk-hr">

                <div class="uk-panel uk-border-rounded-medium uk-overflow-hidden box-shadow simple-title">
                    <a href="#" title="भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत के सारे कोडवर्ड्स बदल दिये" class="uk-display-block">
                        <div class="uk-position-relative">
                            <img class="uk-border-bottom" data-src="https://img.etimg.com/thumb/width-640,height-360,imgsize-225294,resizemode-1,msid-72074484/government-mulls-raising-insurance-cover-on-bank-deposits-to-above-rs-1-lakh-nirmala-sitharaman.jpg" alt="Featured Article" width="400" height="300" uk-img/>
                            <span class="play-icon uk-position-center" uk-icon="icon:triangle-right;ratio:2.0"></span>
                        </div>
                    <h3><span class="red-label uk-text-bold">Sports - </span> भारतीय ख़ुफ़िया एजेंसी से डरी पाकिस्तानी एजेंसी, आतंकियों से बातचीत...</h3></a>
                </div>
                
            </div><!-- section -->

            <div class="section uk-padding-small">
                Outbrain
            </div>
        </div><!-- uk-background-muted -->
    </div><!-- #content -->

    <?php include '_footer.php'; ?>
</div><!-- #wrapper -->
<script src="js/uikit.min.js"></script>
<script src="js/uikit-icons.min.js"></script>
</body>
</html>